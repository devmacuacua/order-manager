...
public class UserController {
  ...
  public Response create(UserRequestDTO dto) {
    if (dto == null || dto.getName() == null || dto.getEmail() == null) {
      return Response.status(Response.Status.BAD_REQUEST).entity("Name and email required").build();
    }

    try {
      UserEntity created = service.create(UserMapper.toEntity(dto));
      return Response.status(Response.Status.CREATED).entity(UserMapper.toResponse(created)).build();
    } catch (Exception e) {
